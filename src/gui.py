...
from src.config import APPLICATION_WIDTH, DEFAULT_MODEL, MODELS, THEME, SCREENSHOT_CAPTURE_HOTKEY, DEFAULT_CHARACTER
...
def build_layout() -> (
    ...
    instructions: sg.Text = create_text_area(
        size=(int(APPLICATION_WIDTH * 0.7), 4),
        key="-INSTRUCTIONS-",
        text="Press 'R' to start recording\n" +
             "Press 'A' to transcribe the recording and provide answers\n" +
             "Press 'V' to paste text from the clipboard and provide answers\n" +
             f"Press '{SCREENSHOT_CAPTURE_HOTKEY.upper()}' to take and transcribe an screenshot and provide answers",
    )
    ...
    character = sg.Input(
        default_text=DEFAULT_CHARACTER,
        k="-CHARACTER_INPUT-",
        s=30,
        tooltip="Enter the character of the AI",
        focus=False,
    )

    screenshot_x_length = sg.Input(
        default_text=1000,
        k="-X_SCREENSHOT_LENGTH_INPUT-",
        s=5,
        tooltip="Screenshot X length (from tip of cursor to the right)",
        focus=False
    )

    screenshot_y_length = sg.Input(
        default_text=500,
        k="-Y_SCREENSHOT_LENGTH_INPUT-",
        s=5,
        tooltip="Screenshot Y length (from tip of cursor to the bottom)",
        focus=False
    )
    ...
    top_frame = create_frame(
        layout=[
            [name("Model"), model],
            [name("AI Character"), character],
            [name("Screenshot X Length"), screenshot_x_length],
            [name("Screenshot Y Length"), screenshot_y_length],
        ],
        key="-TOP_FRAME-",
    )
